...
int main() {
    ...
    codifica c[max_cod];
    size_t j=0, k=0, m=0;
    fp2 = fopen("C:\\Users\\admin\\Desktop\\Robba\\Materiale Polito\\II Anno\\Algoritmi e Programmazione\\2020-2021\\Lab01\\2\\dizionario.txt", "r");
    fscanf(fp2, "%d", &n_cod);
    for(int i=0; i<n_cod; i++){
        fscanf(fp2, "%s %s", c[i].codice, c[i].word);
    }
    fclose(fp2);
    fp1 = fopen("sorgente.txt", "r");
    fp3 = fopen("ricodificato.txt", "w");

    while(fgets(riga, 201, fp1) != NULL) {
        j=0;
        while(j<strlen(riga)) {
            flag = 0;
            for (int i = 0; i < n_cod && flag == 0; i++) {
                if(riga[j] == c[i].word[0]){        // possibile sostituzione
                    k=1;
                    while(k<strlen(c[i].word)) {
