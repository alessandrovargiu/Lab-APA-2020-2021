...
typedef enum command {
    c_acquisizione, c_stampa, o_data, o_tratta, o_partenza, o_arrivo, r_partenza, c_fine
} command;
...
Tratta *ordinamenti[5];             //vettore di puntatori al tipo Tratta con cui gestisco contemporaneamente gli ordinamenti
// il primo ordinamento è quello originale del log
...
void ordina(Tratta *v, int nr){          //funzione wrapper chiamata all'inizio del programma per creare tutti gli ordinamenti richiesti
    ...
    MergeSort(ordinamenti[1], nr, 0);
    MergeSort(ordinamenti[2], nr, 1);
    MergeSort(ordinamenti[3], nr, 2);
    MergeSort(ordinamenti[4], nr, 3);
...
void leggifile(int *nr){
    FILE *fp;
    char nomefile[20];
    Tratta *v = ordinamenti[0];

    printf("Inserisci il nome del file di acquisizione dati: ");
    scanf("%s", nomefile);
    ...
    fclose(fp);
    ordinamenti[0] = malloc(*nr * sizeof(Tratta));
    memcpy(ordinamenti[0], v, *nr * sizeof(Tratta));    //copio il log originale nel primo vettore degli ordinamenti
}
void selezioneDati(command c, int *nr){
    char staz[10];
    switch(c){
        case c_acquisizione:
            leggifile(nr);
            ordina(ordinamenti[0], *nr);               //creo tutti gli ordinamenti possibili
            break;
        case c_stampa:
        ...
        case o_partenza:
            ...
            break;
        case o_arrivo:
            stampalog(ordinamenti[4], *nr);
            break;
        case r_partenza:
            printf("Inserisci una stazione di partenza: \n");
            scanf("%s", staz);
            BinSearch(ordinamenti[3], 0, *nr, staz);  //avendo il vettore ordinato per staz. di partenza opto per la binsearch
            break;
...
command leggiComando(){
    ...
    char *tabella[c_fine+1] = {
            "lettura", "stampa", "data", "tratta", "partenza", "arrivo", "ricerca", "fine"
    };
    char cmd[maxstr];

    printf("Comandi:\nlettura --> lettura dati da file\nstampa --> stampa contenuti del log originale su schermo\nOrdinamenti:\ndata | tratta | partenza | arrivo\n");
